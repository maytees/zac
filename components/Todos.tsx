    ...
    <div className=''>
        <div className="flex-col flex justify-center items-center w-full">
          <div className="flex-row flex justify-evenly mt-10 align-center w-screen">
              ...
              <button className="btn"><label htmlFor="my-modal" className="btn">Create new</label></button>
          </div>
          <div className="flex flex-col flex-wrap align-center justify-between w-3/6">
            {
              todos ?
              (todos.map((todo: TypeTodo) => (
                <Todo key={todo.content} content={todo.content}/>
              ))) : null
            }
          </div>
        </div>
        ...
        <div className="modal">
          <div className="modal-box relative bg-gray-900">
            <div className="flex flex-row justify-between align-center">
              <h1 className="font-bold text-lg text-base-200">Create new todo</h1>
              <label htmlFor="my-modal" className="btn btn-sm btn-circle absolute right-2 top-2">✕</label>
            </div>
            <div className="form-control">
              <label className="input-group mt-5">
                <span className="bg-gray-800 text-white">Text</span>
                <input type="text"
                  value={newTodoValue}
            ...
            <div className="modal-action">
              <button className="btn text-base-200" onClick={createNew}>Create</button>
            </div>
          </div>
        </div>
    </div>
